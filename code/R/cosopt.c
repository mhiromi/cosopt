  ...
  for(i=0;i< samples_number;i++){
    ...
    for(j=0;j<samples[i].random_sample_number;j++){
      ...
      for(k=0;k<samples[i].timepoint_number;k++){
        ...
        for(l=0;l<rt;l+=2){
          nr[l] = samples[i].sigma * norm_rand();
          nr[l+1] = samples[i].sigma * norm_rand();
        }
...
int calculate_minimum_SSR(Sample *samples, int sample_num, double *test_cosine, double *test_cosine_mean, double *ans){
  double i;
  double a,b;
  double distance;
...
void cosopt(double *microarray,double *sigma,double *time_points,int *mrow,int *mcol,double *ans){
  double *test_cosine;
  ...
  calculate_minimum_SSR(samples,gene_num,test_cosine,test_cosine_mean,ans);
}
